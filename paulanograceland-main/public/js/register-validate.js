...
$("#register-form #sponsorNameField").on("focus", function (e) {
    clearInputSponsorFeedBack("#register-form", "#sponsorNameField");
});

$("#register-form #sponsorNameField").on("focusout", function (e) {
...
$("#register-form #sponsorNameField").on("focus", function (e) {
    clearInputFeedBack("#register-form", "#sponsorNameField");
});
...
function sponsorAjaxValidation() {
    sponsorConstant = 0;
    var sponsorName = $("#register-form #sponsorNameField").val();
    if (!validateSponsorName()) {
        sponsorConstant = 0;
        return false;
    }
    $.ajax({
        url: validateSponsorUrl,
        dataType: 'json',
        ...
        success: function (response) {
            $("#register-form #sponsorNameField").get(0).disabled = false;
            if (response.responseCode == 1) {
                addSponsorSuccess("#register-form", "#sponsorNameField", response.firstName + " " + response.lastName);
                sponsorConstant = 1;
            } else if (response.responseCode == 0) {
                addSponsorError("#register-form", "#sponsorNameField", "Sponsor does not exist.");
                sponsorConstant = 0;
            }
        },
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            ...
            //show error message            
            addSponsorError("#register-form", "#sponsorNameField", "");
            sponsorConstant = 0;
            $("#register-form #sponsorNameField").get(0).disabled = false;
        }
    });
    $("#register-form #sponsorNameField").get(0).disabled = true;
    addSponsorLoading("#register-form", "#sponsorNameField");
}
...
$('#register-form').on('submit', function (e) {       
    ...
    if (!validateSponsorName()) {
        if (error == 0) {
            var position = ($("#register-form #sponsorNameField").offset().top) - 70;
            scrollTo(0, position);
        }
        error++;
    }
...
function validateSponsorName() {
    var sponsorName = $("#register-form").find("#sponsorNameField").val();
    if (isEmpty(sponsorName)) {
        addSponsorError("#register-form", "#sponsorNameField", "Please Fill out this field");
        return false;
    } else if (!isLetterOrNumber(sponsorName)) {
        addSponsorError("#register-form", "#sponsorNameField", "Invalid Sponsor Name. No special characters and spaces allowed.");
        return false;
    } else if (!validateLength(sponsorName, 3, 20)) {
        addSponsorError("#register-form", "#sponsorNameField", "Sponsor Name should contain 3 - 20 characters without spaces");
        return false;
    } else {
        return true;
    }
